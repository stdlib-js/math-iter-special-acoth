...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterAcoth, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	values = [ 1.0, 1.5, 2.0 ];
	it = iterAcoth( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse hyperbolic cotangent of each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterAcoth( linspace( 1.0, 10.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 1.0, 10.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = acoth( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse hyperbolic cotangent of each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': acoth( values[ 0 ] ),
			'done': false
		},
		{
			'value': acoth( values[ 1 ] ),
			'done': false
		},
		{
			'value': acoth( values[ 2 ] ),
			'done': false
		},
		{
			'value': acoth( values[ 3 ] ),
			'done': true
		}
	];
